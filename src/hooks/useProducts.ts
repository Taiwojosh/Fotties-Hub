    ...
    const fetchProducts = async () => {
          ...
          Papa.parse(csvText, {
            ...
            complete: (results) => {
              ...
              if (parsedProducts.length > 0) {
                ...
                // If Best Sellers URL is missing, use first 4 products as best sellers
      ...
      if (!WEARS_GOOGLE_SHEET_CSV_URL) {
        ...
        try {
          ...
          console.error("Error fetching/parsing Wears CSV:", err);
          setWears(FALLBACK_WEARS);
          setWearsCategories(['All', ...new Set(FALLBACK_WEARS.map(p => p.category))]);
        }
      }
      ...
      if (BEST_SELLERS_GOOGLE_SHEET_CSV_URL) {
        try {
          ...
          console.error("Error fetching/parsing Best Sellers CSV:", err);
        }
      }

      // We'll just set loading false after a short delay to allow fetch/parse to finish
